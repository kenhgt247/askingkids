...
export const ParentingBlog: React.FC<BlogProps> = ({ user, addNotification, onUpdateUserPoints, initialPostId }) => {
  ...
  const handleLike = () => {
    ...
    if(user && onUpdateUserPoints) {
       onUpdateUserPoints(2); // +2 points for liking
    } else {
  ...
  const handleSubmitComment = () => {
    ...
    if(user && onUpdateUserPoints) {
        onUpdateUserPoints(5); // +5 points for commenting
  ...
  const handleLikeComment = (commentId: string) => {
    ...
    if(user && onUpdateUserPoints) onUpdateUserPoints(1); // +1 point for liking a comment
  };
  ...
  const handleReplySubmit = (parentId: string) => {
    ...
    if(user && onUpdateUserPoints) {
        onUpdateUserPoints(5); // +5 points for replying
  ...
  const handleSubscribe = () => {
    ...
    setTimeout(() => {
       ...
       setNewsletterEmail('');
       if(user && onUpdateUserPoints) {
          onUpdateUserPoints(5); // Reward points for newsletter signup
       }
    }, 1500);
  };
