...
const MathGame: React.FC = () => {
  ...
  const generateProblem = () => {
    ...
    const result = isPlus ? a + b : (a > b ? a - b : b - a); // Avoid negative for kids
    
    // Adjust logic for subtraction to always be positive
      ...
      <div className="grid grid-cols-3 gap-6 w-full max-w-lg">
        {options.map((opt, i) => (
          <button
            ...
            className="py-6 text-4xl font-bold bg-white text-gray-700 rounded-xl shadow-md border-b-4 border-gray-200 hover:border-kid-green hover:bg-green-50 active:translate-y-1 transition-all"
          >
            {opt}
          </button>
        ))}
...
const SpeakingGame: React.FC = () => {
  ...
  const startListening = () => {
    ...
    recognition.onend = () => {
      if (status === 'LISTENING') {
        setStatus('IDLE'); // Reset if silence
      }
    };
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-purple-50">
      <div className="mb-8">
        ...
        {isLoading ? (
          ...
          <div className="bg-white p-8 rounded-2xl shadow-lg border-2 border-kid-purple relative">
             <button 
                onClick={playWord}
                className="absolute top-2 right-2 text-gray-400 hover:text-kid-purple"
                title="Nghe mẫu"
             >
               <Volume2 size={24} />
             </button>
      ...
      <div className="h-24 flex items-center justify-center w-full max-w-md mb-6">
        {status === 'LISTENING' && (
           <div className="flex flex-col items-center animate-pulse text-kid-pink font-bold">
             <Mic className="w-12 h-12 mb-2" />
             Đang nghe...
           </div>
        )}
        {status === 'EVALUATING' && <span className="text-gray-500">Đang chấm điểm... <Sparkles className="inline w-4 h-4 animate-spin"/></span>}
          ...
          <div className={`p-4 rounded-xl border-l-4 w-full ${status === 'SUCCESS' ? 'bg-green-100 border-green-500' : 'bg-red-100 border-red-500'}`}>
             <div className="flex items-center justify-between">
                <div className="text-left">
                  <p className="text-xs text-gray-500 uppercase">Bé nói:</p>
                  <p className="font-bold text-lg">"{spokenText}"</p>
                </div>
                {status === 'SUCCESS' ? <CheckCircle className="text-green-500 w-8 h-8"/> : <XCircle className="text-red-500 w-8 h-8"/>}
             </div>
             <p className="mt-2 text-sm font-medium text-gray-700">{feedback}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-4">
        <button
          ...
          className={`flex items-center gap-2 px-8 py-4 rounded-full text-xl font-bold text-white shadow-lg transform transition-all active:scale-95 ${
            status === 'LISTENING' ? 'bg-gray-400' : 'bg-kid-purple hover:bg-purple-600 hover:shadow-xl'
          }`}
        >
          <Mic size={24} />
          {status === 'RETRY' ? 'Thử lại' : 'Nói ngay'}
        </button>

        {(status === 'SUCCESS' || status === 'RETRY') && (
           <button
           onClick={loadNewWord}
           className="flex items-center gap-2 px-6 py-4 rounded-full text-xl font-bold bg-white text-gray-700 border-2 border-gray-200 hover:border-kid-blue hover:text-kid-blue transition-all"
         >
           Từ tiếp theo <ArrowRight size={20}/>
         </button>
        )}
         {status !== 'SUCCESS' && status !== 'RETRY' && !isLoading && (
             <button
             onClick={loadNewWord}
             className="p-4 rounded-full bg-white text-gray-400 border-2 border-gray-100 hover:text-gray-600 hover:border-gray-300"
             title="Đổi từ khác"
           >
             <RefreshCw size={24} />
           </button>
