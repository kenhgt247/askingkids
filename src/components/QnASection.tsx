...
import { MessageCircle, ThumbsUp, Eye, Star, Award, Zap, Send, Bot, User as UserIcon, Plus, Sparkles, Tag, X, SendHorizontal, Search, Heart, CheckCircle } from 'lucide-react';
    ...
    return (
      <div className="max-w-6xl mx-auto p-4 md:p-6 grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="hidden md:block space-y-6">
           {user ? (
               <div className="bg-gradient-to-r from-kid-yellow to-orange-400 rounded-2xl p-6 text-white shadow-lg">
                  ...
                  <div className="font-bold text-lg flex items-center gap-1 mb-2">
                     <Award size={20}/> {user.rank}
                  </div>
                  <div className="text-sm opacity-90 mb-4">Điểm tích lũy: {user.points}</div>
               </div>
           ) : (
      ...
      <div className="space-y-6 hidden lg:block">
        ...
        <div className="bg-white rounded-2xl shadow-md p-4 sticky top-24">
           ...
           <div className="space-y-1">
             <button onClick={() => setActiveCategory('ALL')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold ${activeCategory === 'ALL' ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50'}`}><Zap size={20} className="text-gray-600"/> Tất cả chủ đề</button>
             <button onClick={() => setActiveCategory(QnACategory.EDUCATION)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold ${activeCategory === QnACategory.EDUCATION ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50'}`}><Award size={20} className="text-kid-blue"/> Giáo dục</button>
             <button onClick={() => setActiveCategory(QnACategory.ABROAD)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold ${activeCategory === QnACategory.ABROAD ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50'}`}><Send size={20} className="text-kid-purple"/> Du học</button>
             <button onClick={() => setActiveCategory(QnACategory.TECH)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold ${activeCategory === QnACategory.TECH ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50'}`}><Bot size={20} className="text-gray-800"/> Công nghệ</button>
             <button onClick={() => setActiveCategory(QnACategory.PARENTS)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold ${activeCategory === QnACategory.PARENTS ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50'}`}><UserIcon size={20} className="text-kid-pink"/> Cha mẹ</button>
             <button onClick={() => setActiveCategory(QnACategory.KIDS)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-bold ${activeCategory === QnACategory.KIDS ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:bg-gray-50'}`}><Star size={20} className="text-kid-yellow"/> Trẻ em</button>
           </div>
        </div>
      </div>
