...
export enum AppView {
  ...
  SEARCH = 'SEARCH' // New view for Global Search
}
...
export const RANK_SYSTEM = [
  ...
  { name: 'Chuyên gia', minPoints: 1000, icon: '👑' } // Highest rank
];

export const calculateRank = (points: number): string => {
  // Sort descending by minPoints to ensure we find the highest threshold first
...
export interface BlogComment {
  ...
  replies: BlogComment[]; // Added nested replies
}
...
export interface Answer {
  ...
  timestamp: string;
  isAccepted?: boolean;
  replies: Answer[]; // Added nested replies for answers
}
...
// User Profile for Gamification (Merged into User but kept for compatibility if needed)
